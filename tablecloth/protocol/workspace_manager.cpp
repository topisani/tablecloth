...
namespace cloth {
  ...
  static const struct workspace_manager_interface workspace_manager_impl = {
    .switch_to = [](wl::client_t*,
                    wl::resource_t* resource,
                    uint32_t ws) { static_cast<WorkspaceManager*>(resource->data)->switch_to(ws); },
    .move_surface =
      [](wl::client_t*, wl::resource_t* resource, wl::resource_t* surface, uint32_t ws) {
        static_cast<WorkspaceManager*>(resource->data)->move_surface(surface, ws);
      }};

  static void bind_workspace_manager(wl::client_t* client,
                                     void* data,
                                     uint32_t version,
                                     uint32_t id)
  {
    ...
    wl::resource_t* resource =
      wl_resource_create(client, &workspace_manager_interface, version, id);
  ...
  WorkspaceManager::WorkspaceManager(Server& server)
    : server(server),
      global(wl_global_create(server.wl_display,
