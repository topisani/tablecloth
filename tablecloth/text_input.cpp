...
namespace cloth {
  ...
  static TextInput* relay_get_focusable_text_input(InputMethodRelay& relay)
  {
    for (auto& text_input : relay.text_inputs)
    {
  ...
  static TextInput* relay_get_focused_text_input(InputMethodRelay& relay)
  {
    for (auto& text_input : relay.text_inputs)
    {
  ...
  {
    InputMethodRelay* relay = wl_container_of(listener, relay, input_method_commit);
    TextInput* text_input = relay_get_focused_text_input(relay);
    if (!text_input) {
      return;
    }
    wlr::input_method_v2_t* context = data;
  ...
  static void text_input_set_pending_focused_surface(TextInput* text_input, wlr::surface_t* surface)
  {
  ...
  static void text_input_clear_pending_focused_surface(TextInput* text_input)
  {
  ...
  {
    InputMethodRelay* relay = wl_container_of(listener, relay, input_method_destroy);
    wlr::input_method_v2_t* context = data;
    assert(context == relay->input_method);
  ...
  static void relay_send_im_done(InputMethodRelay* relay, wlr::text_input_v3_t* input)
  {
    wlr::input_method_v2_t* input_method = relay->input_method;
    if (!input_method) {
  ...
  static TextInput* text_input_to_roots(InputMethodRelay* relay, wlr::text_input_v3_t* text_input)
  {
    TextInput* roots_text_input = NULL;
  ...
  {
    InputMethodRelay* relay = wl_container_of(listener, relay, text_input_enable);
    if (relay->input_method == NULL) {
      wlr_log(WLR_INFO, "Enabling text input when input method is gone");
      return;
    }
    TextInput* text_input = text_input_to_roots(relay, (wlr::text_input_v3_t*) data);
  ...
  {
    InputMethodRelay* relay = wl_container_of(listener, relay, text_input_commit);
    TextInput* text_input = text_input_to_roots(relay, (wlr::text_input_v3_t*) data);
  ...
  static void relay_disable_text_input(InputMethodRelay* relay, TextInput* text_input)
  {
    if (relay->input_method == NULL) {
  ...
  {
    InputMethodRelay* relay = wl_container_of(listener, relay, text_input_disable);
    TextInput* text_input = text_input_to_roots(relay, (wlr::text_input_v3_t*) data);
    relay_disable_text_input(relay, text_input);
  }
  static void handle_text_input_destroy(struct wl_listener* listener, void* data)
  {
    InputMethodRelay* relay = wl_container_of(listener, relay, text_input_destroy);
    TextInput* text_input = text_input_to_roots(relay, (wlr::text_input_v3_t*) data);
    if (text_input->input->current_enabled) {
      relay_disable_text_input(relay, text_input);
    }
  ...
  static void handle_pending_focused_surface_destroy(struct wl_listener* listener, void* data)
  {
    ...
    wlr::surface_t* surface = data;
    assert(text_input->pending_focused_surface == surface);
    text_input->pending_focused_surface = NULL;
  }
  TextInput* roots_text_input_create(InputMethodRelay* relay, wlr::text_input_v3_t* text_input)
  {
  ...
  {
    InputMethodRelay* relay = wl_container_of(listener, relay, text_input_new);
    wlr::text_input_v3_t* wlr_text_input = data;
    if (relay->seat->seat != wlr_text_input->seat) {
      return;
    }
    TextInput* text_input = roots_text_input_create(relay, wlr_text_input);
    if (!text_input) {
      return;
    }
    wl_list_insert(&relay->text_inputs, &text_input->link);
  }
  static void relay_handle_input_method(struct wl_listener* listener, void* data)
  {
    InputMethodRelay* relay = wl_container_of(listener, relay, input_method_new);
    wlr::input_method_v2_t* input_method = data;
    ...
    TextInput* text_input = relay_get_focusable_text_input(relay);
    if (text_input) {
      wlr_text_input_v3_send_enter(text_input->input, text_input->pending_focused_surface);
      text_input_clear_pending_focused_surface(text_input);
    }
  }
  void InputMethodRelay::init(Seat* seat, InputMethodRelay* relay)
  {
    relay->seat = seat;
    ...
    wl_signal_add(&seat->input->server->desktop->input_method->events.input_method,
                  &relay->input_method_new);
  }
  void InputMethodRelay::set_focus(InputMethodRelay* relay, wlr::surface_t* surface)
  {
    ...
    wl_list_for_each(text_input, &relay->text_inputs, link)
    {
      ...
      } else if (surface && wl_resource_get_client(text_input->input->resource) ==
