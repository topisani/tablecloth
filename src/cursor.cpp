...
namespace cloth {
  ...
  Cursor::Cursor(Seat& seat, wlr::cursor_t* cursor) noexcept
   : seat(seat), wlr_cursor (cursor)
  {
    // add input signals
    ...
    on_request_set_cursor = [this](void* data) {
      ...
      handle_request_set_cursor(event);
    };
  }
  ...
  static void seat_view_deco_motion(struct roots_seat_view* view, double deco_sx, double deco_sy)
  {
    ...
    if (view->has_button_grab) {
      ...
      if (is_titlebar) {
        wlr_xcursor_manager_set_cursor_image(cursor->xcursor_manager, cursor->default_xcursor,
                                             cursor->cursor);
      } else if (edges) {
        const char* resize_name = wlr_xcursor_get_resize_name(edges);
        wlr_xcursor_manager_set_cursor_image(cursor->xcursor_manager, resize_name, cursor->cursor);
      }
    }
  }
  ...
  static void seat_view_deco_button(struct roots_seat_view* view,
                                    double sx,
                                    double sy,
                                    uint32_t button,
                                    uint32_t state)
  ...
  static void roots_passthrough_cursor(struct roots_cursor* cursor, uint32_t time)
  {
    double sx, sy;
    ...
    struct wlr_surface* surface =
      desktop_surface_at(desktop, cursor->cursor->x, cursor->cursor->y, &sx, &sy, &view);
    ...
    if (view) {
      struct roots_seat_view* seat_view = roots_seat_view_from_view(seat, view);
  ...
  static void roots_cursor_update_position(struct roots_cursor* cursor, uint32_t time)
  {
    ...
    switch (cursor->mode) {
    case ROOTS_CURSOR_PASSTHROUGH: roots_passthrough_cursor(cursor, time); break;
    ...
    case ROOTS_CURSOR_RESIZE:
      ...
      if (view != NULL) {
        ...
        view_move_resize(view, x, y, width < 1 ? 1 : width, height < 1 ? 1 : height);
      }
      break;
    case ROOTS_CURSOR_ROTATE:
      ...
      if (view != NULL) {
        ...
        int ux = cursor->offs_x - ox, uy = cursor->offs_y - oy;
        int vx = cursor->cursor->x - ox, vy = cursor->cursor->y - oy;
                                        ...
                                        double ly)
  {
    ...
    double sx, sy;
    struct roots_view* view;
    struct wlr_surface* surface = desktop_surface_at(desktop, lx, ly, &sx, &sy, &view);
    ...
    if (state == WLR_BUTTON_PRESSED && view && roots_seat_has_meta_pressed(seat)) {
      ...
      uint32_t edges;
      switch (button) {
      case BTN_LEFT: roots_seat_begin_move(seat, view); break;
      case BTN_RIGHT:
        edges = 0;
        ...
        break;
      case BTN_MIDDLE: roots_seat_begin_rotate(seat, view); break;
      }
    } else {
      ...
      if (state == WLR_BUTTON_RELEASED && cursor->mode != ROOTS_CURSOR_PASSTHROUGH) {
      ...
      case WLR_BUTTON_PRESSED:
        ...
        if (surface && wlr_surface_is_layer_surface(surface)) {
          struct wlr_layer_surface* layer = wlr_layer_surface_from_wlr_surface(surface);
  ...
  {
    roots_cursor_press_button(cursor, event->device, event->time_msec, event->button, event->state,
                              cursor->cursor->x, cursor->cursor->y);
  }

  void roots_cursor_handle_axis(struct roots_cursor* cursor, struct wlr_event_pointer_axis* event)
  {
    wlr_seat_pointer_notify_axis(cursor->seat->seat, event->time_msec, event->orientation,
                                      ...
                                      struct wlr_event_touch_down* event)
  {
    ...
    double lx, ly;
    wlr_cursor_absolute_to_layout_coords(cursor->cursor, event->device, event->x, event->y, &lx,
                                         &ly);

    double sx, sy;
    struct wlr_surface* surface = desktop_surface_at(desktop, lx, ly, &sx, &sy, NULL);

    uint32_t serial = 0;
    if (surface && roots_seat_allow_input(cursor->seat, surface->resource)) {
      serial = wlr_seat_touch_notify_down(cursor->seat->seat, surface, event->time_msec,
    ...
    if (serial && wlr_seat_touch_num_points(cursor->seat->seat) == 1) {
      ...
      roots_cursor_press_button(cursor, event->device, event->time_msec, BTN_LEFT, 1, lx, ly);
    }
  }

  void roots_cursor_handle_touch_up(struct roots_cursor* cursor, struct wlr_event_touch_up* event)
  {
    ...
    if (wlr_seat_touch_num_points(cursor->seat->seat) == 1) {
      roots_cursor_press_button(cursor, event->device, event->time_msec, BTN_LEFT, 0,
                                cursor->seat->touch_x, cursor->seat->touch_y);
    }
                                        ...
                                        struct wlr_event_touch_motion* event)
  {
    ...
    if (surface && roots_seat_allow_input(cursor->seat, surface->resource)) {
      wlr_seat_touch_point_focus(cursor->seat->seat, surface, event->time_msec, event->touch_id, sx,
                                 sy);
      wlr_seat_touch_notify_motion(cursor->seat->seat, event->time_msec, event->touch_id, sx, sy);
    } else {
      wlr_seat_touch_point_clear_focus(cursor->seat->seat, event->time_msec, event->touch_id);
    }
  ...
  {
    roots_cursor_press_button(cursor, event->device, event->time_msec, BTN_LEFT, event->state,
                              cursor->cursor->x, cursor->cursor->y);
  }

  void roots_cursor_handle_request_set_cursor(
    ...
    struct wlr_seat_pointer_request_set_cursor_event* event)
  {
    ...
    if (event->seat_client->client != focused_client || cursor->mode != ROOTS_CURSOR_PASSTHROUGH) {
