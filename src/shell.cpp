...
#include <libweston-4/config-parser.h>
...
static void activate(struct desktop_shell* shell,
...
static void surface_rotate(struct shell_surface* surface, struct wl_seat* seat);
...
static bool shell_surface_is_top_fullscreen(struct shell_surface* shsurf)
{
  ...
  if (wl_list_empty(&shell->fullscreen_layer.link)) return false;

  top_fs_es =
    wl_container_of(shell->fullscreen_layer.link.next, struct weston_surface, layer_link);
...
static void destroy_shell_grab_shsurf(struct wl_listener* listener, void* data)
{
  ...
  grab = container_of(listener, struct shell_grab, shsurf_destroy_listener);
                             ...
                             enum desktop_shell_cursor cursor)
{
  ...
  weston_pointer_set_focus(pointer, &shell->grab_surface->surface, wl_fixed_from_int(0),
                       wl_fixed_from_int(0));
}

static void shell_grab_end(struct shell_grab* grab)
{
  if (grab->shsurf) wl_list_remove(&grab->shsurf_destroy_listener.link);

  weston_pointer_end_grab(grab->pointer);
}

static void center_on_output(struct weston_surface* surface, struct weston_output* output);

static enum weston_keyboard_modifier get_modifier(char* modifier)
{
  if (!modifier) return MODIFIER_SUPER;
...
{
  if (!animation) return ANIMATION_NONE;
...
static void shell_configuration(struct desktop_shell* shell, const char* config_file)
{
  char* path = NULL;
  int duration = 60;
  ...
  struct config_key shell_keys[] = {
    ...
    {"num-workspaces", CONFIG_KEY_UNSIGNED_INTEGER, &num_workspaces},
  };
...
{
  struct focus_state* state = container_of(listener, struct focus_state, seat_destroy_listener);
...
{
  struct focus_state* state = container_of(listener, struct focus_state, surface_destroy_listener);
  struct desktop_shell* shell;
  ...
  {
    if (surface == state->keyboard_focus) continue;
...
static struct focus_state* focus_state_create(struct weston_seat* seat, struct workspace* ws)
{
  ...
  wl_signal_add(&seat->seat.destroy_signal, &state->seat_destroy_listener);
...
static struct focus_state* ensure_focus_state(struct desktop_shell* shell, struct weston_seat* seat)
{
  ...
  if (&state->link == &ws->focus_list) state = focus_state_create(seat, ws);

  return state;
}

static void restore_focus_state(struct desktop_shell* shell, struct workspace* ws)
{
  ...
  {
    surface = state->keyboard_focus ? &state->keyboard_focus->surface : NULL;

    wl_keyboard_set_focus(state->seat->seat.keyboard, surface);
  }
}

static void replace_focus_state(struct desktop_shell* shell,
                                struct workspace* ws,
                                struct weston_seat* seat)
{
  struct focus_state* state;
  ...
  wl_list_for_each(state, &ws->focus_list, link)
  {
    if (state->seat == seat) {
      ...
      state->keyboard_focus = (struct weston_surface*) surface;
      return;
    }
  }
}

static void drop_focus_state(struct desktop_shell* shell,
                             struct workspace* ws,
                             struct weston_surface* surface)
{
  struct focus_state* state;
...
static void workspace_destroy(struct workspace* ws)
{
  ...
  wl_list_for_each_safe(state, next, &ws->focus_list, link) focus_state_destroy(state);
...
static void seat_destroyed(struct wl_listener* listener, void* data)
{
  ...
  struct focus_state *state, *next;
  struct workspace* ws = container_of(listener, struct workspace, seat_destroyed_listener);
...
static struct workspace* get_workspace(struct desktop_shell* shell, unsigned int index)
{
...
static void surface_translate(struct weston_surface* surface, double d)
{
  ...
  if (wl_list_empty(&transform->link))
    wl_list_insert(surface->geometry.transformation_list.prev, &shsurf->workspace_transform.link);

  weston_matrix_init(&shsurf->workspace_transform.matrix);
  weston_matrix_translate(&shsurf->workspace_transform.matrix, 0.0, d, 0.0);
...
static void broadcast_current_workspace_state(struct desktop_shell* shell)
{
  struct wl_resource* resource;

  wl_list_for_each(resource, &shell->workspaces.client_list, link)
    workspace_manager_send_state(resource, shell->workspaces.current, shell->workspaces.num);
}
...
static void animate_workspace_change_frame(struct weston_animation* animation,
                                           ...
                                           uint32_t msecs)
{
  struct desktop_shell* shell = container_of(animation, struct desktop_shell, workspaces.animation);
    ...
    if (shell->workspaces.anim_current == 0.0)
      ...
      shell->workspaces.anim_timestamp = msecs -
                                         ...
                                         (asin(1.0 - shell->workspaces.anim_current) *
                                          DEFAULT_WORKSPACE_CHANGE_ANIMATION_LENGTH * M_2_PI);
  }
                                     ...
                                     struct workspace* to)
{
  ...
  output = container_of(shell->compositor->output_list.next, struct weston_output, link);
  wl_list_insert(&output->animation_list, &shell->workspaces.animation.link);
...
static void update_workspace(struct desktop_shell* shell,
                             unsigned int index,
                             struct workspace* from,
                             struct workspace* to)
{
...
static void change_workspace(struct desktop_shell* shell, unsigned int index)
{
  struct workspace* from;
  struct workspace* to;

  if (index == shell->workspaces.current) return;

  /* Don't change workspace when there is any fullscreen surfaces. */
  if (!wl_list_empty(&shell->fullscreen_layer.surface_list)) return;

  from = get_current_workspace(shell);
  to = get_workspace(shell, index);

  if (shell->workspaces.anim_from == to && shell->workspaces.anim_to == from) {
    restore_focus_state(shell, to);
  ...
  if (shell->workspaces.anim_to != NULL)
    finish_workspace_change_animation(shell, shell->workspaces.anim_from,
...
static bool workspace_has_only(struct workspace* ws, struct weston_surface* surface)
{
  struct wl_list* list = &ws->layer.surface_list;
  struct wl_list* e;

  if (wl_list_empty(list)) return false;

  e = list->next;

  if (e->next != list) return false;

  return container_of(e, struct weston_surface, layer_link) == surface;
}

static void move_surface_to_workspace(struct desktop_shell* shell,
                                      struct weston_surface* surface,
                                      uint32_t workspace)
{
  struct workspace* from;
  ...
  if (workspace == shell->workspaces.current) return;

  if (workspace >= shell->workspaces.num) workspace = shell->workspaces.num - 1;
  ...
  wl_list_for_each(
    seat, &shell->compositor->seat_list,
    link) if (seat->has_keyboard && seat->keyboard.keyboard.focus == &surface->surface)
    wl_keyboard_set_focus(&seat->keyboard.keyboard, NULL);

  weston_surface_damage_below(surface);
}

static void take_surface_to_workspace_by_seat(struct desktop_shell* shell,
                                              struct wl_seat* wl_seat,
                                              unsigned int index)
{
  struct weston_seat* seat = (struct weston_seat*) wl_seat;
  struct weston_surface* surface = (struct weston_surface*) wl_seat->keyboard->focus;
  struct shell_surface* shsurf;
  struct workspace* from;
  struct workspace* to;
  struct focus_state* state;

  if (surface == NULL || index == shell->workspaces.current) return;
  ...
  if (workspace_is_empty(from) && workspace_has_only(to, surface))
    update_workspace(shell, index, from, to);
  else {
    shsurf = get_shell_surface(surface);
    if (wl_list_empty(&shsurf->workspace_transform.link))
      wl_list_insert(&shell->workspaces.anim_sticky_list, &shsurf->workspace_transform.link);
  ...
  if (state != NULL) state->keyboard_focus = surface;
}
                                           ...
                                           uint32_t workspace)
{
  ...
  struct weston_surface* surface = (struct weston_surface*) surface_resource;
...
static void bind_workspace_manager(struct wl_client* client,
                                   void* data,
                                   uint32_t version,
                                   uint32_t id)
{
  ...
  resource = wl_client_add_object(client, &workspace_manager_interface,
                                  &workspace_manager_implementation, id, shell);
...
static void move_grab_motion(struct weston_pointer_grab* grab,
                             uint32_t time,
                             wl_fixed_t x,
                             wl_fixed_t y)
{
...
static void move_grab_button(struct weston_pointer_grab* grab,
                             uint32_t time,
                             uint32_t button,
                             uint32_t state_w)
{
  struct shell_grab* shell_grab = container_of(grab, struct shell_grab, grab);
  struct weston_pointer* pointer = grab->pointer;
  enum weston_pointer_button_state state = state_w;
...
static int surface_move(struct shell_surface* shsurf, struct weston_seat* ws)
{
  struct weston_move_grab* move;

  if (!shsurf) return -1;

  if (shsurf->type == SHELL_SURFACE_FULLSCREEN) return 0;

  move = malloc(sizeof *move);
  if (!move) return -1;

  move->dx = wl_fixed_from_double(shsurf->surface->geometry.x) - ws->seat.pointer->grab_x;
  move->dy = wl_fixed_from_double(shsurf->surface->geometry.y) - ws->seat.pointer->grab_y;

  shell_grab_start(&move->base, &move_grab_interface, shsurf, ws->seat.pointer,
                   DESKTOP_SHELL_CURSOR_MOVE);
                               ...
                               uint32_t serial)
{
  ...
  if (surface_move(shsurf, ws) < 0) wl_resource_post_no_memory(resource);
}
...
static void resize_grab_motion(struct weston_pointer_grab* grab,
                               uint32_t time,
                               wl_fixed_t x,
                               wl_fixed_t y)
{
  ...
  weston_surface_from_global_fixed(shsurf->surface, pointer->grab_x, pointer->grab_y, &from_x,
                                   &from_y);
  weston_surface_from_global_fixed(shsurf->surface, pointer->x, pointer->y, &to_x, &to_y);
  ...
  shsurf->client->send_configure(shsurf->surface, resize->edges, width, height);
}

static void send_configure(struct weston_surface* surface,
                           uint32_t edges,
                           int32_t width,
                           int32_t height)
{
...
static const struct weston_shell_client shell_client = {send_configure};

static void resize_grab_button(struct weston_pointer_grab* grab,
                               uint32_t time,
                               uint32_t button,
                               uint32_t state_w)
{
...
static int surface_resize(struct shell_surface* shsurf, struct weston_seat* ws, uint32_t edges)
{
  struct weston_resize_grab* resize;

  if (shsurf->type == SHELL_SURFACE_FULLSCREEN || shsurf->type == SHELL_SURFACE_MAXIMIZED) return 0;
  ...
  shell_grab_start(&resize->base, &resize_grab_interface, shsurf, ws->seat.pointer, edges);
                                 ...
                                 uint32_t edges)
{
  ...
  if (shsurf->type == SHELL_SURFACE_FULLSCREEN) return;

  if (ws->seat.pointer->button_count == 0 || ws->seat.pointer->grab_serial != serial ||
      ws->seat.pointer->focus != &shsurf->surface->surface)
    return;

  if (surface_resize(shsurf, ws, edges) < 0) wl_resource_post_no_memory(resource);
}
...
static void busy_cursor_grab_motion(struct weston_pointer_grab* grab,
                                    uint32_t time,
                                    int32_t x,
                                    int32_t y)
{}

static void busy_cursor_grab_button(struct weston_pointer_grab* base,
                                    uint32_t time,
                                    uint32_t button,
                                    uint32_t state)
{
  struct shell_grab* grab = (struct shell_grab*) base;
  struct shell_surface* shsurf;
  struct weston_surface* surface = (struct weston_surface*) grab->grab.pointer->current;
  struct weston_seat* seat = (struct weston_seat*) grab->grab.pointer->seat;
...
static void set_busy_cursor(struct shell_surface* shsurf, struct weston_pointer* pointer)
{
  ...
  shell_grab_start(grab, &busy_cursor_grab_interface, shsurf, pointer, DESKTOP_SHELL_CURSOR_BUSY);
}

static void end_busy_cursor(struct shell_surface* shsurf, struct weston_pointer* pointer)
{
...
static void ping_timer_destroy(struct shell_surface* shsurf)
{
  if (!shsurf || !shsurf->ping_timer) return;

  if (shsurf->ping_timer->source) wl_event_source_remove(shsurf->ping_timer->source);

  free(shsurf->ping_timer);
  shsurf->ping_timer = NULL;
}

static int ping_timeout_handler(void* data)
{
  ...
  wl_list_for_each(seat, &shsurf->surface->compositor->seat_list,
                   link) if (seat->seat.pointer->focus == &shsurf->surface->surface)
    set_busy_cursor(shsurf, seat->seat.pointer);

  return 1;
}

static void ping_handler(struct weston_surface* surface, uint32_t serial)
{
  struct shell_surface* shsurf = get_shell_surface(surface);
  ...
  if (!shsurf) return;
  if (!shsurf->resource.client) return;

  if (shsurf->surface == shsurf->shell->grab_surface) return;

  if (!shsurf->ping_timer) {
    shsurf->ping_timer = malloc(sizeof *shsurf->ping_timer);
    if (!shsurf->ping_timer) return;

    shsurf->ping_timer->serial = serial;
    loop = wl_display_get_event_loop(surface->compositor->wl_display);
    shsurf->ping_timer->source = wl_event_loop_add_timer(loop, ping_timeout_handler, shsurf);
...
static void handle_pointer_focus(struct wl_listener* listener, void* data)
{
  struct weston_pointer* pointer = data;
  struct weston_surface* surface = (struct weston_surface*) pointer->focus;
  struct weston_compositor* compositor;
  struct shell_surface* shsurf;
  uint32_t serial;
...
static void create_pointer_focus_listener(struct weston_seat* seat)
{
                               ...
                               uint32_t serial)
{
  ...
  if (shsurf->ping_timer == NULL) /* Just ignore unsolicited pong. */
    return;
  ...
  if (shsurf->ping_timer->serial == serial) {
    ...
    if (was_unresponsive) {
      ...
      wl_list_for_each(seat, &ec->seat_list, link)
      {
        ...
        if (pointer->focus == &shell->grab_surface->surface &&
            pointer->current == &shsurf->surface->surface)
                                    ...
                                    const char* klass)
{
  ...
  free(shsurf->klass);
  shsurf->klass = strdup(klass);
}

static struct weston_output* get_default_output(struct weston_compositor* compositor)
{
  return container_of(compositor->output_list.next, struct weston_output, link);
}

static void shell_unset_fullscreen(struct shell_surface* shsurf)
{
  struct workspace* ws;
  /* undo all fullscreen things here */
  if (shsurf->fullscreen.type == WL_SHELL_SURFACE_FULLSCREEN_METHOD_DRIVER &&
      shell_surface_is_top_fullscreen(shsurf)) {
    weston_output_switch_mode(shsurf->fullscreen_output, shsurf->fullscreen_output->origin);
  }
  shsurf->fullscreen.type = WL_SHELL_SURFACE_FULLSCREEN_METHOD_DEFAULT;
  shsurf->fullscreen.framerate = 0;
  wl_list_remove(&shsurf->fullscreen.transform.link);
  wl_list_init(&shsurf->fullscreen.transform.link);
  if (shsurf->fullscreen.black_surface) weston_surface_destroy(shsurf->fullscreen.black_surface);
  shsurf->fullscreen.black_surface = NULL;
  shsurf->fullscreen_output = NULL;
  weston_surface_set_position(shsurf->surface, shsurf->saved_x, shsurf->saved_y);
...
static void shell_unset_maximized(struct shell_surface* shsurf)
{
  struct workspace* ws;
...
static int reset_shell_surface_type(struct shell_surface* surface)
{
  switch (surface->type) {
  case SHELL_SURFACE_FULLSCREEN: shell_unset_fullscreen(surface); break;
  case SHELL_SURFACE_MAXIMIZED: shell_unset_maximized(surface); break;
  case SHELL_SURFACE_NONE:
  case SHELL_SURFACE_TOPLEVEL:
  case SHELL_SURFACE_TRANSIENT:
  case SHELL_SURFACE_POPUP: break;
  }
...
static void set_surface_type(struct shell_surface* shsurf)
{
  ...
  switch (shsurf->type) {
  case SHELL_SURFACE_TOPLEVEL: break;
  case SHELL_SURFACE_TRANSIENT:
    weston_surface_set_position(surface, pes->geometry.x + shsurf->transient.x,
...
static void set_transient(struct shell_surface* shsurf,
                          struct weston_surface* parent,
                          int x,
                          int y,
                          uint32_t flags)
...
static int get_output_panel_height(struct desktop_shell* shell, struct weston_output* output)
{
  struct weston_surface* surface;
  int panel_height = 0;
                                        ...
                                        struct wl_resource* output_resource)
{
  ...
  shsurf->client->send_configure(shsurf->surface, edges, shsurf->output->width,
                                 shsurf->output->height - panel_height);

  shsurf->next_type = SHELL_SURFACE_MAXIMIZED;
}

static void black_surface_configure(struct weston_surface* es,
                                    int32_t sx,
                                    int32_t sy,
                                    int32_t width,
                                    int32_t height);

static struct weston_surface* create_black_surface(struct weston_compositor* ec,
...
static void shell_configure_fullscreen(struct shell_surface* shsurf)
{
  ...
  if (!shsurf->fullscreen.black_surface)
    shsurf->fullscreen.black_surface = create_black_surface(
      surface->compositor, surface, output->x, output->y, output->width, output->height);
  ...
  case WL_SHELL_SURFACE_FULLSCREEN_METHOD_DEFAULT:
    if (surface->buffer_ref.buffer) center_on_output(surface, shsurf->fullscreen_output);
    break;
  case WL_SHELL_SURFACE_FULLSCREEN_METHOD_SCALE:
    ...
    surface_aspect = (float) surface->geometry.width / (float) surface->geometry.height;
    ...
    wl_list_remove(&shsurf->fullscreen.transform.link);
    wl_list_insert(&surface->geometry.transformation_list, &shsurf->fullscreen.transform.link);
  ...
  case WL_SHELL_SURFACE_FULLSCREEN_METHOD_DRIVER:
    if (shell_surface_is_top_fullscreen(shsurf)) {
      struct weston_mode mode = {0, surface->geometry.width, surface->geometry.height,
                                 shsurf->fullscreen.framerate};

      if (weston_output_switch_mode(output, &mode) == 0) {
        weston_surface_configure(shsurf->fullscreen.black_surface, output->x, output->y,
    ...
    break;
  case WL_SHELL_SURFACE_FULLSCREEN_METHOD_FILL: break;
  default: break;
  }
}

/* make the fullscreen and black surface at the top */
static void shell_stack_fullscreen(struct shell_surface* shsurf)
{
  struct weston_output* output = shsurf->fullscreen_output;
  struct weston_surface* surface = shsurf->surface;
  ...
  wl_list_remove(&surface->layer_link);
  wl_list_insert(&shell->fullscreen_layer.surface_list, &surface->layer_link);
...
static void shell_map_fullscreen(struct shell_surface* shsurf)
{
  shell_stack_fullscreen(shsurf);
  shell_configure_fullscreen(shsurf);
}

static void set_fullscreen(struct shell_surface* shsurf,
                           uint32_t method,
                           uint32_t framerate,
                           struct weston_output* output)
{
  ...
  shsurf->client->send_configure(shsurf->surface, 0, shsurf->output->width, shsurf->output->height);
}
...
static void destroy_shell_seat(struct wl_listener* listener, void* data)
{
  struct shell_seat* shseat = container_of(listener, struct shell_seat, seat_destroy_listener);
...
static struct shell_seat* create_shell_seat(struct weston_seat* seat)
{
  struct shell_seat* shseat;
...
static struct shell_seat* get_shell_seat(struct weston_seat* seat)
{
  ...
  if (listener == NULL) return create_shell_seat(seat);

  return container_of(listener, struct shell_seat, seat_destroy_listener);
}
...
{
  struct weston_pointer* pointer = grab->pointer;
  struct shell_seat* shseat = container_of(grab, struct shell_seat, popup_grab.grab);
...
static void popup_grab_motion(struct weston_pointer_grab* grab,
                              uint32_t time,
                              wl_fixed_t sx,
                              wl_fixed_t sy)
{
  ...
  if (resource) weston_pointer_send_motion(resource, time, sx, sy);
}

static void popup_grab_button(struct weston_pointer_grab* grab,
                              uint32_t time,
                              uint32_t button,
                              uint32_t state_w)
{
  ...
  if (resource) {
    ...
    weston_pointer_send_button(resource, serial, time, button, state);
  } else if (state == WL_POINTER_BUTTON_STATE_RELEASED &&
             (shseat->popup_grab.initial_up || time - shseat->seat->pointer.grab_time > 500)) {
    popup_grab_end(grab->pointer);
  }

  if (state == WL_POINTER_BUTTON_STATE_RELEASED) shseat->popup_grab.initial_up = 1;
}
...
static void add_popup_grab(struct shell_surface* shsurf, struct shell_seat* shseat)
{
  ...
  if (wl_list_empty(&shseat->popup_grab.surfaces_list)) {
     ...
     * popups already open. */
    if (shseat->seat->pointer.button_count > 0) shseat->popup_grab.initial_up = 0;
...
static void remove_popup_grab(struct shell_surface* shsurf)
{
...
static void shell_map_popup(struct shell_surface* shsurf)
{
  struct shell_seat* shseat = shsurf->popup.shseat;
  struct weston_surface* es = shsurf->surface;
...
static const struct wl_shell_surface_interface shell_surface_implementation = {
  shell_surface_pong,         shell_surface_move,          shell_surface_resize,
  shell_surface_set_toplevel, shell_surface_set_transient, shell_surface_set_fullscreen,
  shell_surface_set_popup,    shell_surface_set_maximized, shell_surface_set_title,
  shell_surface_set_class};

static void destroy_shell_surface(struct shell_surface* shsurf)
{
...
static void shell_handle_surface_destroy(struct wl_listener* listener, void* data)
{
  struct shell_surface* shsurf =
    container_of(listener, struct shell_surface, surface_destroy_listener);
...
static void shell_surface_configure(struct weston_surface*, int32_t, int32_t, int32_t, int32_t);
...
static struct shell_surface* create_shell_surface(void* shell,
                                                  struct weston_surface* surface,
                                                  const struct weston_shell_client* client)
{
  struct shell_surface* shsurf;
                                    ...
                                    struct wl_resource* surface_resource)
{
  ...
  shsurf->resource.object.implementation = (void (**)(void)) & shell_surface_implementation;
  shsurf->resource.data = shsurf;

  wl_client_add_resource(client, &shsurf->resource);
}

static const struct wl_shell_interface shell_implementation = {shell_get_shell_surface};

static void shell_fade(struct desktop_shell* shell, enum fade_type type);
...
{
  struct desktop_shell* shell = container_of(proc, struct desktop_shell, screensaver.process);

  proc->pid = 0;

  if (shell->locked) weston_compositor_sleep(shell->compositor);
}

static void launch_screensaver(struct desktop_shell* shell)
{
  if (shell->screensaver.binding) return;
  ...
  weston_client_launch(shell->compositor, &shell->screensaver.process, shell->screensaver.path,
                       handle_screensaver_sigchld);
}

static void terminate_screensaver(struct desktop_shell* shell)
{
  if (shell->screensaver.process.pid == 0) return;

  kill(shell->screensaver.process.pid, SIGTERM);
}

static void configure_static_surface(struct weston_surface* es,
                                     struct weston_layer* layer,
                                     int32_t width,
                                     int32_t height)
{
                                         ...
                                         struct wl_resource* surface_resource)
{
  ...
  desktop_shell_send_configure(resource, 0, surface_resource, surface->output->width,
                               surface->output->height);
}

static void panel_configure(struct weston_surface* es,
                            int32_t sx,
                            int32_t sy,
                            int32_t width,
                            int32_t height)
...
static void desktop_shell_set_panel(struct wl_client* client,
                                    struct wl_resource* resource,
                                    struct wl_resource* output_resource,
                                    struct wl_resource* surface_resource)
{
...
static void lock_surface_configure(struct weston_surface* surface,
                                   int32_t sx,
                                   int32_t sy,
                                   int32_t width,
                                   int32_t height)
{
  ...
  if (!weston_surface_is_mapped(surface)) {
    wl_list_insert(&shell->lock_layer.surface_list, &surface->layer_link);
...
{
  struct desktop_shell* shell = container_of(listener, struct desktop_shell, lock_surface_listener);

  weston_log("lock surface gone\n");
  shell->lock_surface = NULL;
}

static void desktop_shell_set_lock_surface(struct wl_client* client,
                                           struct wl_resource* resource,
                                           struct wl_resource* surface_resource)
{
  struct desktop_shell* shell = resource->data;
  ...
  wl_signal_add(&surface_resource->destroy_signal, &shell->lock_surface_listener);

  surface->configure = lock_surface_configure;
  surface->configure_private = shell;
}

static void resume_desktop(struct desktop_shell* shell)
{
  struct workspace* ws = get_current_workspace(shell);
  ...
  wl_list_insert(&shell->compositor->cursor_layer.link, &shell->fullscreen_layer.link);
  wl_list_insert(&shell->fullscreen_layer.link, &shell->panel_layer.link);
  if (shell->showing_input_panels) {
    wl_list_insert(&shell->panel_layer.link, &shell->input_panel_layer.link);
    wl_list_insert(&shell->input_panel_layer.link, &ws->layer.link);
  } else {
    wl_list_insert(&shell->panel_layer.link, &ws->layer.link);
  }
...
static void desktop_shell_unlock(struct wl_client* client, struct wl_resource* resource)
{
  ...
  if (shell->locked) resume_desktop(shell);
}

static void desktop_shell_set_grab_surface(struct wl_client* client,
                                           struct wl_resource* resource,
                                           struct wl_resource* surface_resource)
{
  struct desktop_shell* shell = resource->data;
...
static const struct desktop_shell_interface desktop_shell_implementation = {
  desktop_shell_set_background, desktop_shell_set_panel, desktop_shell_set_lock_surface,
  desktop_shell_unlock, desktop_shell_set_grab_surface};
...
static enum shell_surface_type get_shell_surface_type(struct weston_surface* surface)
{
  ...
  shsurf = get_shell_surface(surface);
  if (!shsurf) return SHELL_SURFACE_NONE;
  return shsurf->type;
}

static void move_binding(struct wl_seat* seat, uint32_t time, uint32_t button, void* data)
{
  struct weston_surface* surface = (struct weston_surface*) seat->pointer->focus;
  struct shell_surface* shsurf;
...
static void resize_binding(struct wl_seat* seat, uint32_t time, uint32_t button, void* data)
{
  struct weston_surface* surface = (struct weston_surface*) seat->pointer->focus;
  uint32_t edges = 0;
  int32_t x, y;
  ...
  weston_surface_from_global(surface, wl_fixed_to_int(seat->pointer->grab_x),
                             wl_fixed_to_int(seat->pointer->grab_y), &x, &y);
...
static void surface_opacity_binding(struct wl_seat* seat,
                                    uint32_t time,
                                    uint32_t axis,
                                    wl_fixed_t value,
                                    void* data)
{
  ...
  if (surface->alpha > 1.0) surface->alpha = 1.0;
  if (surface->alpha < step) surface->alpha = step;

  weston_surface_geometry_dirty(surface);
  weston_surface_damage(surface);
}

static void do_zoom(struct wl_seat* seat,
                    uint32_t time,
                    uint32_t key,
                    uint32_t axis,
                    wl_fixed_t value)
{
  ...
  wl_list_for_each(output, &compositor->output_list, link)
  {
    if (pixman_region32_contains_point(&output->region, wl_fixed_to_double(seat->pointer->x),
                                       wl_fixed_to_double(seat->pointer->y), NULL)) {
      if (key == KEY_PAGEUP)
        ...
        increment = output->zoom.increment * -wl_fixed_to_double(value) / 20.0;
      else
        increment = 0;
...
static void zoom_axis_binding(struct wl_seat* seat,
                              uint32_t time,
                              uint32_t axis,
                              wl_fixed_t value,
                              void* data)
...
static void zoom_key_binding(struct wl_seat* seat, uint32_t time, uint32_t key, void* data)
{
  do_zoom(seat, time, key, 0, 0);
}

static void terminate_binding(struct wl_seat* seat, uint32_t time, uint32_t key, void* data)
{
...
static void rotate_grab_motion(struct weston_pointer_grab* grab,
                               uint32_t time,
                               wl_fixed_t x,
                               wl_fixed_t y)
{
  struct rotate_grab* rotate = container_of(grab, struct rotate_grab, base.grab);
  struct weston_pointer* pointer = grab->pointer;
  ...
  if (r > 20.0f) {
    struct weston_matrix* matrix = &shsurf->rotation.transform.matrix;
  ...
  if (dposx != 0.0f || dposy != 0.0f) {
    weston_surface_set_position(surface, surface->geometry.x + dposx, surface->geometry.y + dposy);
  }
...
static void rotate_grab_button(struct weston_pointer_grab* grab,
                               uint32_t time,
                               uint32_t button,
                               uint32_t state_w)
{
  ...
  if (pointer->button_count == 0 && state == WL_POINTER_BUTTON_STATE_RELEASED) {
    if (shsurf) weston_matrix_multiply(&shsurf->rotation.rotation, &rotate->rotation);
...
static void surface_rotate(struct shell_surface* surface, struct wl_seat* seat)
{
  ...
  weston_surface_to_global_float(surface->surface, surface->surface->geometry.width / 2,
                                 surface->surface->geometry.height / 2, &rotate->center.x,
                                 &rotate->center.y);
  ...
  shell_grab_start(&rotate->base, &rotate_grab_interface, surface, seat->pointer,
                   DESKTOP_SHELL_CURSOR_ARROW);
}

static void rotate_binding(struct wl_seat* seat, uint32_t time, uint32_t button, void* data)
{
  struct weston_surface* base_surface = (struct weston_surface*) seat->pointer->focus;
  struct shell_surface* surface;

  if (base_surface == NULL) return;
...
static void lower_fullscreen_layer(struct desktop_shell* shell)
{
  struct workspace* ws;
  struct weston_surface *surface, *prev;

  ws = get_current_workspace(shell);
  wl_list_for_each_reverse_safe(surface, prev, &shell->fullscreen_layer.surface_list, layer_link)
                     ...
                     struct weston_seat* seat)
{
  ...
  wl_list_remove(&state->surface_destroy_listener.link);
  wl_signal_add(&es->surface.resource.destroy_signal, &state->surface_destroy_listener);
...
static bool is_black_surface(struct weston_surface* es, struct weston_surface** fs_surface)
{
  if (es->configure == black_surface_configure) {
    if (fs_surface) *fs_surface = (struct weston_surface*) es->configure_private;
    return true;
  }
  return false;
}

static void click_to_activate_binding(struct wl_seat* seat,
                                      uint32_t time,
                                      uint32_t button,
                                      void* data)
{
  ...
  if (is_black_surface(focus, &upper)) focus = upper;

  if (get_shell_surface_type(focus) == SHELL_SURFACE_NONE) return;

  if (seat->pointer->grab == &seat->pointer->default_grab) activate(shell, focus, ws);
}

static void lock(struct desktop_shell* shell)
{
  struct workspace* ws = get_current_workspace(shell);
  ...
  if (shell->showing_input_panels) wl_list_remove(&shell->input_panel_layer.link);
  wl_list_remove(&ws->layer.link);
  wl_list_insert(&shell->compositor->cursor_layer.link, &shell->lock_layer.link);
...
static void unlock(struct desktop_shell* shell)
{
...
static void shell_fade_done(struct weston_surface_animation* animation, void* data)
{
  struct desktop_shell* shell = data;
  ...
  case FADE_IN:
    ...
    break;
  case FADE_OUT: lock(shell); break;
  }
}
...
static void shell_fade(struct desktop_shell* shell, enum fade_type type)
{
  ...
  switch (type) {
  case FADE_IN: tint = 0.0; break;
  case FADE_OUT: tint = 1.0; break;
  default: weston_log("shell: invalid fade type\n"); return;
  }
  ...
  if (shell->fade.animation)
    weston_fade_update(shell->fade.animation, shell->fade.surface->alpha, tint, 30.0);
  else
    shell->fade.animation =
      weston_fade_run(shell->fade.surface, 1.0 - tint, tint, 30.0, shell_fade_done, shell);
}

static void idle_handler(struct wl_listener* listener, void* data)
{
  struct desktop_shell* shell = container_of(listener, struct desktop_shell, idle_listener);

  shell_fade(shell, FADE_OUT);
  /* lock() is called from shell_fade_done() */
}

static void wake_handler(struct wl_listener* listener, void* data)
{
  struct desktop_shell* shell = container_of(listener, struct desktop_shell, wake_listener);

  unlock(shell);
}

static void show_input_panels(struct wl_listener* listener, void* data)
{
  struct desktop_shell* shell =
  ...
  if (shell->showing_input_panels) return;

  shell->showing_input_panels = true;

  if (!shell->locked) wl_list_insert(&shell->panel_layer.link, &shell->input_panel_layer.link);

  wl_list_for_each_safe(surface, next, &shell->input_panel.surfaces, link)
  {
    ws = surface->surface;
    wl_list_insert(&shell->input_panel_layer.surface_list, &ws->layer_link);
...
static void hide_input_panels(struct wl_listener* listener, void* data)
{
  struct desktop_shell* shell =
  ...
  if (!shell->locked) wl_list_remove(&shell->input_panel_layer.link);

  wl_list_for_each_safe(surface, next, &shell->input_panel_layer.surface_list, layer_link)
...
static void weston_surface_set_initial_position(struct weston_surface* surface,
                                                struct desktop_shell* shell)
{
  struct weston_compositor* compositor = shell->compositor;
  int ix = 0, iy = 0;
  ...
  if (!target_output) {
    weston_surface_set_position(surface, 10 + random() % 400, 10 + random() % 400);
    return;
  }
  ...
  range_y = (target_output->height - panel_height) - surface->geometry.height;
...
static void map(struct desktop_shell* shell,
                struct weston_surface* surface,
                int32_t width,
                int32_t height,
                int32_t sx,
                int32_t sy)
{
  ...
  switch (surface_type) {
  case SHELL_SURFACE_TOPLEVEL: weston_surface_set_initial_position(surface, shell); break;
  case SHELL_SURFACE_FULLSCREEN:
  ...
  case SHELL_SURFACE_MAXIMIZED:
    ...
    weston_surface_set_position(surface, shsurf->output->x, shsurf->output->y + panel_height);
    break;
  case SHELL_SURFACE_POPUP: shell_map_popup(shsurf); break;
  case SHELL_SURFACE_NONE:
    weston_surface_set_position(surface, surface->geometry.x + sx, surface->geometry.y + sy);
    break;
  default:;
  }
  ...
  switch (surface_type) {
  case SHELL_SURFACE_TRANSIENT:
    if (shsurf->transient.flags == WL_SHELL_SURFACE_TRANSIENT_INACTIVE) break;
  case SHELL_SURFACE_TOPLEVEL:
  case SHELL_SURFACE_FULLSCREEN:
  case SHELL_SURFACE_MAXIMIZED:
    if (!shell->locked) {
      wl_list_for_each(seat, &compositor->seat_list, link) activate(shell, surface, seat);
    }
    break;
  default: break;
  }
  ...
  if (surface_type == SHELL_SURFACE_TOPLEVEL) {
    ...
    case ANIMATION_FADE: weston_fade_run(surface, 0.0, 1.0, 200.0, NULL, NULL); break;
    case ANIMATION_ZOOM: weston_zoom_run(surface, 0.8, 1.0, NULL, NULL); break;
    default: break;
    }
  }
}

static void configure(struct desktop_shell* shell,
                      struct weston_surface* surface,
                      float x,
                      float y,
                      int32_t width,
                      int32_t height)
{
  ...
  shsurf = get_shell_surface(surface);
  if (shsurf) surface_type = shsurf->type;
  ...
  case SHELL_SURFACE_MAXIMIZED:
    ...
    surface->geometry.y = surface->output->y + get_output_panel_height(shell, surface->output);
    break;
  case SHELL_SURFACE_TOPLEVEL: break;
  default: break;
  }
...
static void shell_surface_configure(struct weston_surface* es,
                                    int32_t sx,
                                    int32_t sy,
                                    int32_t width,
                                    int32_t height)
{
  ...
  if (shsurf->next_type != SHELL_SURFACE_NONE && shsurf->type != shsurf->next_type) {
  ...
  if (!weston_surface_is_mapped(es)) {
  ...
  } else if (type_changed || sx != 0 || sy != 0 || es->geometry.width != width ||
    ...
    configure(shell, es, es->geometry.x + to_x - from_x, es->geometry.y + to_y - from_y, width,
              height);
  }
}
...
{
  uint32_t time;
  struct desktop_shell* shell = container_of(process, struct desktop_shell, child.process);
...
static void launch_desktop_shell_process(void* data)
{
  ...
  shell->child.client = weston_client_launch(shell->compositor, &shell->child.process, shell_exe,
...
static void bind_shell(struct wl_client* client, void* data, uint32_t version, uint32_t id)
{
  ...
  wl_client_add_object(client, &wl_shell_interface, &shell_implementation, id, shell);
}
...
static void bind_desktop_shell(struct wl_client* client, void* data, uint32_t version, uint32_t id)
{
  ...
  resource = wl_client_add_object(client, &desktop_shell_interface, &desktop_shell_implementation,
                                  id, shell);
...
static void screensaver_configure(struct weston_surface* surface,
                                  int32_t sx,
                                  int32_t sy,
                                  int32_t width,
                                  int32_t height)
{
  ...
  if (wl_list_empty(&surface->layer_link)) {
    wl_list_insert(shell->lock_layer.surface_list.prev, &surface->layer_link);
    weston_surface_update_transform(surface);
    wl_event_source_timer_update(shell->screensaver.timer, shell->screensaver.duration);
    shell_fade(shell, FADE_IN);
  }
}

static void screensaver_set_surface(struct wl_client* client,
                                    struct wl_resource* resource,
                                    struct wl_resource* surface_resource,
                                    struct wl_resource* output_resource)
{
...
static const struct screensaver_interface screensaver_implementation = {screensaver_set_surface};
...
static void bind_screensaver(struct wl_client* client, void* data, uint32_t version, uint32_t id)
{
  ...
  resource =
    wl_client_add_object(client, &screensaver_interface, &screensaver_implementation, id, shell);
...
static void input_panel_configure(struct weston_surface* surface,
                                  int32_t sx,
                                  int32_t sy,
                                  int32_t width,
                                  int32_t height)
{
  ...
  if (!weston_surface_is_mapped(surface)) return;
  ...
  weston_surface_configure(surface, surface->output->x + x, surface->output->y + y, width, height);
}

static void destroy_input_panel_surface(struct input_panel_surface* input_panel_surface)
{
...
static void input_panel_handle_surface_destroy(struct wl_listener* listener, void* data)
{
  struct input_panel_surface* ipsurface =
    container_of(listener, struct input_panel_surface, surface_destroy_listener);

  if (ipsurface->resource.client) {
    wl_resource_destroy(&ipsurface->resource);
  } else {
    wl_signal_emit(&ipsurface->resource.destroy_signal, &ipsurface->resource);
    destroy_input_panel_surface(ipsurface);
  }
}
static struct input_panel_surface* create_input_panel_surface(struct desktop_shell* shell,
                                                              struct weston_surface* surface)
{
  struct input_panel_surface* input_panel_surface;

  input_panel_surface = calloc(1, sizeof *input_panel_surface);
  if (!input_panel_surface) return NULL;
...
static void input_panel_surface_set_toplevel(struct wl_client* client,
                                             struct wl_resource* resource,
                                             uint32_t position)
{
...
static void destroy_input_panel_surface_resource(struct wl_resource* resource)
{
...
static void input_panel_get_input_panel_surface(struct wl_client* client,
                                                struct wl_resource* resource,
                                                uint32_t id,
                                                struct wl_resource* surface_resource)
{
  ...
  ipsurf->resource.object.implementation = (void (**)(void)) & input_panel_surface_implementation;
...
static void unbind_input_panel(struct wl_resource* resource)
{
  struct desktop_shell* shell = resource->data;
...
static void bind_input_panel(struct wl_client* client, void* data, uint32_t version, uint32_t id)
{
  struct desktop_shell* shell = data;
  struct wl_resource* resource;

  resource =
    wl_client_add_object(client, &input_panel_interface, &input_panel_implementation, id, shell);
...
static void switcher_next(struct switcher* switcher)
{
  ...
  wl_list_for_each(surface, &ws->layer.surface_list, layer_link)
  {
    ...
    case SHELL_SURFACE_MAXIMIZED:
      ...
      if (prev == switcher->current) next = surface;
      prev = surface;
      surface->alpha = 0.25;
      weston_surface_geometry_dirty(surface);
      weston_surface_damage(surface);
  ...
  wl_signal_add(&next->surface.resource.destroy_signal, &switcher->listener);
...
{
  struct switcher* switcher = container_of(listener, struct switcher, listener);
...
static void switcher_destroy(struct switcher* switcher)
{
  ...
  if (switcher->current)
    activate(switcher->shell, switcher->current, (struct weston_seat*) keyboard->seat);
  wl_list_remove(&switcher->listener.link);
  wl_keyboard_end_grab(keyboard);
  if (weston_keyboard->input_method_resource) keyboard->grab = &weston_keyboard->input_method_grab;
  free(switcher);
}
                         ...
                         uint32_t state_w)
{
  ...
  if (key == KEY_TAB && state == WL_KEYBOARD_KEY_STATE_PRESSED) switcher_next(switcher);
}

static void switcher_modifier(struct wl_keyboard_grab* grab,
                              uint32_t serial,
                              ...
                              uint32_t group)
{
  ...
  if ((seat->modifier_state & switcher->shell->binding_modifier) == 0) switcher_destroy(switcher);
}
...
static void switcher_binding(struct wl_seat* seat, uint32_t time, uint32_t key, void* data)
{
  struct desktop_shell* shell = data;
...
static void backlight_binding(struct wl_seat* seat, uint32_t time, uint32_t key, void* data)
{
  struct weston_compositor* compositor = data;
  struct weston_output* output;
  ...
  if (!output->set_backlight) return;
  ...
  if (backlight_new < 5) backlight_new = 5;
  if (backlight_new > 255) backlight_new = 255;

  output->backlight_current = backlight_new;
  output->set_backlight(output, output->backlight_current);
}

static void fan_debug_repaint_binding(struct wl_seat* seat, uint32_t time, uint32_t key, void* data)
{
  struct desktop_shell* shell = data;
  struct weston_compositor* compositor = shell->compositor;
...
static void debug_binding_key(struct wl_keyboard_grab* grab,
                              uint32_t time,
                              uint32_t key,
                              uint32_t state)
{
  ...
  if (state == WL_KEYBOARD_KEY_STATE_RELEASED) {
    ...
    for (i = 0; i < 2; i++)
      if (key == db->key[i]) db->key_released[i] = 1;
  ...
  if (check_binding) {
    ...
    if (weston_compositor_run_debug_binding(ec, db->seat, time, key, state)) {
...
static void debug_binding_modifiers(struct wl_keyboard_grab* grab,
                                    uint32_t serial,
                                    uint32_t mods_depressed,
                                    uint32_t mods_latched,
                                    uint32_t mods_locked,
...
struct wl_keyboard_grab_interface debug_binding_keyboard_grab = {debug_binding_key,
                                                                 debug_binding_modifiers};

static void debug_binding(struct wl_seat* seat, uint32_t time, uint32_t key, void* data)
{
...
static void force_kill_binding(struct wl_seat* seat, uint32_t time, uint32_t key, void* data)
{
...
static void workspace_up_binding(struct wl_seat* seat, uint32_t time, uint32_t key, void* data)
{
  struct desktop_shell* shell = data;
...
static void workspace_down_binding(struct wl_seat* seat, uint32_t time, uint32_t key, void* data)
{
  struct desktop_shell* shell = data;
  unsigned int new_index = shell->workspaces.current;

  if (shell->locked) return;
  if (new_index < shell->workspaces.num - 1) new_index++;

  change_workspace(shell, new_index);
}

static void workspace_f_binding(struct wl_seat* seat, uint32_t time, uint32_t key, void* data)
{
  struct desktop_shell* shell = data;
  ...
  if (new_index >= shell->workspaces.num) new_index = shell->workspaces.num - 1;

  change_workspace(shell, new_index);
}

static void workspace_move_surface_up_binding(struct wl_seat* seat,
                                              uint32_t time,
                                              uint32_t key,
                                              void* data)
{
...
static void workspace_move_surface_down_binding(struct wl_seat* seat,
                                                uint32_t time,
                                                uint32_t key,
                                                void* data)
{
...
{
  struct desktop_shell* shell = container_of(listener, struct desktop_shell, destroy_listener);
  struct workspace** ws;
  ...
  wl_array_for_each(ws, &shell->workspaces.array) workspace_destroy(*ws);
...
static void shell_add_bindings(struct weston_compositor* ec, struct desktop_shell* shell)
{
  uint32_t mod;
  int i, num_workspace_bindings;

  /* fixed bindings */
  weston_compositor_add_key_binding(ec, KEY_BACKSPACE, MODIFIER_CTRL | MODIFIER_ALT,
                                    terminate_binding, ec);
  weston_compositor_add_button_binding(ec, BTN_LEFT, 0, click_to_activate_binding, shell);
  weston_compositor_add_axis_binding(ec, WL_POINTER_AXIS_VERTICAL_SCROLL,
                                     MODIFIER_SUPER | MODIFIER_ALT, surface_opacity_binding, NULL);
  weston_compositor_add_axis_binding(ec, WL_POINTER_AXIS_VERTICAL_SCROLL, MODIFIER_SUPER,
                                     zoom_axis_binding, NULL);

  /* configurable bindings */
  mod = shell->binding_modifier;
  weston_compositor_add_key_binding(ec, KEY_PAGEUP, mod, zoom_key_binding, NULL);
  weston_compositor_add_key_binding(ec, KEY_PAGEDOWN, mod, zoom_key_binding, NULL);
  weston_compositor_add_button_binding(ec, BTN_LEFT, mod, move_binding, shell);
  weston_compositor_add_button_binding(ec, BTN_MIDDLE, mod, resize_binding, shell);
  weston_compositor_add_button_binding(ec, BTN_RIGHT, mod, rotate_binding, NULL);
  weston_compositor_add_key_binding(ec, KEY_TAB, mod, switcher_binding, shell);
  weston_compositor_add_key_binding(ec, KEY_F9, mod, backlight_binding, ec);
  weston_compositor_add_key_binding(ec, KEY_BRIGHTNESSDOWN, 0, backlight_binding, ec);
  weston_compositor_add_key_binding(ec, KEY_F10, mod, backlight_binding, ec);
  weston_compositor_add_key_binding(ec, KEY_BRIGHTNESSUP, 0, backlight_binding, ec);
  weston_compositor_add_key_binding(ec, KEY_K, mod, force_kill_binding, shell);
  weston_compositor_add_key_binding(ec, KEY_UP, mod, workspace_up_binding, shell);
  weston_compositor_add_key_binding(ec, KEY_DOWN, mod, workspace_down_binding, shell);
  weston_compositor_add_key_binding(ec, KEY_UP, mod | MODIFIER_SHIFT,
                                    workspace_move_surface_up_binding, shell);
  weston_compositor_add_key_binding(ec, KEY_DOWN, mod | MODIFIER_SHIFT,
                                    workspace_move_surface_down_binding, shell);

  /* Add bindings for mod+F[1-6] for workspace 1 to 6. */
  if (shell->workspaces.num > 1) {
    num_workspace_bindings = shell->workspaces.num;
    if (num_workspace_bindings > 6) num_workspace_bindings = 6;
    for (i = 0; i < num_workspace_bindings; i++)
      weston_compositor_add_key_binding(ec, KEY_F1 + i, mod, workspace_f_binding, shell);
  }

  /* Debug bindings */
  weston_compositor_add_key_binding(ec, KEY_SPACE, mod | MODIFIER_SHIFT, debug_binding, shell);
  weston_compositor_add_debug_binding(ec, KEY_F, fan_debug_repaint_binding, shell);
}
                          ...
                          const char* config_file)
{
  ...
  if (wl_display_add_global(ec->wl_display, &wl_shell_interface, shell, bind_shell) == NULL)
    return -1;

  if (wl_display_add_global(ec->wl_display, &desktop_shell_interface, shell, bind_desktop_shell) ==
      NULL)
    return -1;

  if (wl_display_add_global(ec->wl_display, &screensaver_interface, shell, bind_screensaver) ==
      NULL)
    return -1;

  if (wl_display_add_global(ec->wl_display, &input_panel_interface, shell, bind_input_panel) ==
      NULL)
    return -1;

  if (wl_display_add_global(ec->wl_display, &workspace_manager_interface, shell,
  ...
  shell->screensaver.timer = wl_event_loop_add_timer(loop, screensaver_timeout, shell);

  wl_list_for_each(seat, &ec->seat_list, link) create_pointer_focus_listener(seat);
